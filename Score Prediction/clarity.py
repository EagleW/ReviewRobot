...
from model.RNN_torch import Clarity
from sklearn.metrics import accuracy_score, mean_squared_error
from loader.utils import create_clarity
from loader.data_loader import load_clarity
from loader.dataset import Clarity_Dataset, Clarity_Processor
...
parser.add_argument(
    "--model_dp", default="models/clarity_models/",
...
if args.load:
    state = pickle.load(open(args.data_path + '/clarity_dataset.pth', 'rb'))
    mappings = state['mappings']
    r_dataset = state['r_dataset']
    v_dataset = state['v_dataset']
    t_dataset = state['t_dataset']
else:
    words, r_dataset = load_clarity(parameters['data'] + '/CLARITY_train.json', True)
    mappings = create_clarity(words, parameters['freq'])
    v_dataset = load_clarity(parameters['data'] + '/CLARITY_valid.json')
    t_dataset = load_clarity(parameters['data'] + '/CLARITY_test.json')
    state = {
        'mappings': mappings,
        'r_dataset': r_dataset,
        'v_dataset': v_dataset,
    ...
    pickle.dump(state, open(args.data_path + '/clarity_dataset.pth', "wb"))
...
train_dataset = Clarity_Dataset(r_dataset, word2id, parameters['max_len'])
valid_dataset = Clarity_Dataset(v_dataset, word2id, parameters['max_len'])
test_dataset = Clarity_Dataset(t_dataset, word2id, parameters['max_len'])

apro_proc = Clarity_Processor()
...
model = Clarity(embed_layer_word, **parameters)
...
for epoch in range(num_epochs):
    ...
    model.train(True)
    epoch_loss = 0
    for batch_idx, (batch_txts, batch_lens, batch_tgt, batch_ids, _) in enumerate(train_loader):
        batch_txts =batch_txts.to(device)
        batch_tgt = batch_tgt.to(device)
        prob = model(batch_txts, batch_lens)
    ...
    dev_loss = 0
    model.eval()
    for batch_idx, (batch_txts, batch_lens, batch_tgt, batch_ids, _) in enumerate(val_loader):
        batch_txts =batch_txts.to(device)
        batch_tgt = batch_tgt.to(device)
        prob = model(batch_txts, batch_lens)
        batch_loss = criterion(prob, batch_tgt)
...
aspect_all_ys_ = []
aspect_all_ys_avg = []
for batch_idx, (batch_txts, batch_lens, batch_tgt, batch_ids, batch_avg) in enumerate(test_loader):
    batch_txts =batch_txts.to(device)
    batch_tgt = batch_tgt.to(device)
    prob = model(batch_txts, batch_lens)
