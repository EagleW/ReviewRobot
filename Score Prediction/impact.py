...
if args.load:
    state = pickle.load(open(args.data_path + '/impact_dataset.pth', 'rb'))
    mappings = state['mappings']
    r_dataset = state['r_dataset']
    v_dataset = state['v_dataset']
    t_dataset = state['t_dataset']
else:
    words, r_dataset = load_impact(parameters['data'] + '/IMPACT_train.json', True)
    mappings = create_impact(words, parameters['freq'])
    v_dataset = load_impact(parameters['data'] + '/IMPACT_valid.json')
    t_dataset = load_impact(parameters['data'] + '/IMPACT_test.json')
    state = {
        'mappings': mappings,
        'r_dataset': r_dataset,
        'v_dataset': v_dataset,
    ...
    pickle.dump(state, open(args.data_path + '/impact_dataset.pth', "wb"))
...
train_dataset = Impact_Dataset(r_dataset, word2id, f1_bins, parameters['max_len'])
valid_dataset = Impact_Dataset(v_dataset, word2id, f1_bins, parameters['max_len'])
...
for epoch in range(num_epochs):
    ...
    model.train(True)
    epoch_loss = 0
    for batch_idx, (batch_txts, batch_lens, batch_f1s, batch_tgt, batch_ids, _) in enumerate(train_loader):
    ...
    dev_loss = 0
    model.eval()
    for batch_idx, (batch_txts, batch_lens, batch_f1s, batch_tgt, batch_ids, _) in enumerate(val_loader):
        batch_txts =batch_txts.to(device)
        batch_f1s = batch_f1s.to(device)
        batch_tgt = batch_tgt.to(device)
        prob = model(batch_txts, batch_lens, batch_f1s)
