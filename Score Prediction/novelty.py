...
from loader.utils import create_novelty
...
parser.add_argument(
    "--model_dp", default="models/novelty_models/",
...
if args.load:
    state = pickle.load(open(args.data_path + '/novelty_dataset.pth', 'rb'))
    mappings = state['mappings']
    r_dataset = state['r_dataset']
    v_dataset = state['v_dataset']
    t_dataset = state['t_dataset']
else:
    words, r_dataset = load_appro_sound_novelty(parameters['data'] + '/ORIGINALITY_train.json', True)
    mappings = create_novelty(words, parameters['freq'])
    v_dataset = load_appro_sound_novelty(parameters['data'] + '/ORIGINALITY_valid.json')
    t_dataset = load_appro_sound_novelty(parameters['data'] + '/ORIGINALITY_test.json')
    state = {
        'mappings': mappings,
        'r_dataset': r_dataset,
        'v_dataset': v_dataset,
    ...
    pickle.dump(state, open(args.data_path + '/novelty_dataset.pth', "wb"))
...
train_dataset = Appro_Sound_Novelty_Dataset(r_dataset, word2id, f1_bins, f2_bins, parameters['max_len'])
valid_dataset = Appro_Sound_Novelty_Dataset(v_dataset, word2id, f1_bins, f2_bins, parameters['max_len'])
...
optimizer = torch.optim.RMSprop(model.parameters(), args.lr_rate, weight_decay=0.99)
