...
state = pickle.load(open('data/related_dataset.pth', 'rb'))
...
state = torch.load('models/related_models/best/best_dev_model.pth.tar')
...
path_new = 'paper/related/%s.json'
apro_proc = Related_Processor()
for year in years:
    t_dataset = load_related('scores/MEANINGFUL_COMPARISON/' + year + '.json')
    test_dataset = Related_Dataset(t_dataset, word2id, f1_bins, f2_bins, parameters['max_len'])
