...
state = pickle.load(open('data/novelty_dataset.pth', 'rb'))
...
state = torch.load('models/novelty_models/best/best_dev_model.pth.tar')
...
years = ['acl_2017']
path_new = 'paper/novelty/%s.json'
apro_proc = Appro_Sound_Novelty_Processor()
for year in years:
    t_dataset = load_appro_sound_novelty('scores/ORIGINALITY/' + year + '.json')
    test_dataset = Appro_Sound_Novelty_Dataset(t_dataset, word2id, f1_bins, f2_bins, parameters['max_len'])
