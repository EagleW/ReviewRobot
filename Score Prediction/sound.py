...
from loader.utils import create_sound
...
parser.add_argument(
    "--model_dp", default="models/sound_models/",
...
if args.load:
    state = pickle.load(open(args.data_path + '/sound_dataset.pth', 'rb'))
    mappings = state['mappings']
    r_dataset = state['r_dataset']
    v_dataset = state['v_dataset']
    t_dataset = state['t_dataset']
else:
    words, r_dataset = load_appro_sound_novelty(parameters['data'] + '/SOUNDNESS_CORRECTNESS_train.json', True)
    mappings = create_sound(words, parameters['freq'])
    v_dataset = load_appro_sound_novelty(parameters['data'] + '/SOUNDNESS_CORRECTNESS_valid.json')
    t_dataset = load_appro_sound_novelty(parameters['data'] + '/SOUNDNESS_CORRECTNESS_test.json')
    state = {
        'mappings': mappings,
        'r_dataset': r_dataset,
        'v_dataset': v_dataset,
    ...
    pickle.dump(state, open(args.data_path + '/sound_dataset.pth', "wb"))
